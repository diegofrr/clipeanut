...
export default function Sidebar() {
  ...
  return hasSidebar(pathname) ? (
    <nav className="bottom-0 w-full h-16 md:h-auto md:w-20 lg:w-32 fixed left-0 md:top-16 bg-white dark:bg-black p-0 md:py-6 z-50">
      <div className="w-full h-full flex flex-col justify-center md:justify-start items-center">
        <ul className="flex flex-row w-full md:w-auto md:flex-col justify-around md:justify-start items-center gap-6">
          {links.map((link) => (
            <Tooltip
              key={link.name}
              content={link.name}
